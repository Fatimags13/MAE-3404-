...
class PumpCurve_GUI_Class(Ui_Form, qtw.QWidget): #class for PumpCurve_GUI inherits
from Ui_Form & qtw.QWidget
...
self.AssignSignals() #connects 'clicked' signals from buttons to proper
slots
self.FilePath=os.getcwd() #gets the current working directory
self.FileName=""
#these are for matplotlib using pyqt
self.canvas = FigureCanvasQTAgg(Figure(figsize=(5, 3),tight_layout=True,
frameon=True))
...
"""
Here is where I pass a list of display widgets to the controller that get
passed along to the view so that
